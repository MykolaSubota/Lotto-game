...
def unique_player_name(players: list[Player], new_player_name: str):
    for p in players:
        if p.name == new_player_name:
            print('A player with that name already exists!')
            return False
    return True


def can_cross_out_number(player_response: str, number: int, card_numbers: Card):
    if player_response == 'y' and (number in card_numbers.numbers):
        return True
    return False


def not_number_in_card(player_response: str, number: int, card_numbers: Card):
    if player_response == 'n' and (number not in card_numbers.numbers):
        return True
    return False
    ...
    for number in range(number_of_players):
        ...
        if not computer:
            while True:
                ...
                if unique_player_name(players, player_name):
                    break
        else:
            computers += 1
                ...
                if player.is_computer:
                    ...
                    while not switch:
                        ...
                        if player_response == 'y' or player_response == 'n':
                            switch = True
                    if can_cross_out_number(player_response, number, player.player_card):
                        player.player_card.numbers[player.player_card.numbers.index(number)] = '--'
                        player.points += 1
                    elif not_number_in_card(player_response, number, player.player_card):
                        player.points += 1
                    else:
                        player.plays = False
